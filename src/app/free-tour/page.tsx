          ...
          <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm mb-12">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div className="text-center">
                <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-3" style={{background: 'var(--color-secondary)'}}><span className="text-xl font-bold" style={{color: 'var(--color-primary)'}}>1</span></div>
                <h3 className="font-semibold mb-2" style={{color: 'var(--color-primary)'}}>Reserva gratis</h3>
                <p className="text-sm text-slate-600">Reservas online sin pagar nada.</p>
              </div>
              <div className="text-center">
                <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-3" style={{background: 'var(--color-secondary)'}}><span className="text-xl font-bold" style={{color: 'var(--color-primary)'}}>2</span></div>
                <h3 className="font-semibold mb-2" style={{color: 'var(--color-primary)'}}>Haces el tour</h3>
                <p className="text-sm text-slate-600">2-3 horas caminando con un guia local.</p>
              </div>
              <div className="text-center">
                <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-3" style={{background: 'var(--color-secondary)'}}><span className="text-xl font-bold" style={{color: 'var(--color-primary)'}}>3</span></div>
                <h3 className="font-semibold mb-2" style={{color: 'var(--color-primary)'}}>Dejas propina</h3>
                <p className="text-sm text-slate-600">Al final pagas lo que creas justo.</p>
              </div>
            </div>
          </div>
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm text-slate-600">
                      <div><span className="text-slate-400">Duracion:</span> {tour.duracion}</div>
                      <div><span className="text-slate-400">Horarios:</span> {tour.horarios}</div>
                      <div><span className="text-slate-400">Idiomas:</span> {tour.idiomas}</div>
                      <div><span className="text-slate-400">Propina:</span> {tour.propina}</div>
                    </div>
                    <p className="text-sm text-slate-500 mt-2">{tour.punto}</p>
                  </div>
                  <a href={tour.url} target="_blank" rel="noopener noreferrer" className="btn-secondary text-sm inline-flex items-center gap-2 whitespace-nowrap hover:scale-105 transition-all">Reservar</a>
                </div>
              </div>
            ))}
          </div>
          <div className="mt-12 rounded-2xl p-8" style={{background: 'var(--color-primary)'}}>
            <div className="md:flex md:items-center md:justify-between gap-6">
              <div className="mb-6 md:mb-0">
                ...
                <p className="text-white/80">Nuestros itinerarios te dan la misma info pero tu decides cuando parar.</p>
              </div>
              <Link href="/itinerarios" className="btn-primary inline-flex items-center gap-2 whitespace-nowrap bg-white hover:scale-105 transition-all" style={{color: 'var(--color-primary)'}}>Ver itinerarios</Link>
            </div>
          </div>
        </div>
      </section>
