...
const emptyHighlight: GuideHighlight = { time: '', place: '', desc: '' };
    ...
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl border-2 border-slate-200 p-8 space-y-8">
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={() => setFormData({ ...formData, highlights: [...formData.highlights, emptyHighlight] })}
          ...
          {formData.highlights.map((highlight, index) => (
            <div key={`${highlight.time}-${index}`} className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
              <div className="grid md:grid-cols-3 gap-3">
                ...
                <button
                  type="button"
                  onClick={() => setFormData({ ...formData, highlights: formData.highlights.filter((_, i) => i !== index) })}
