      ...
      <section className="py-16" style={{background: 'var(--color-secondary)'}}>
        <div className="max-w-6xl mx-auto px-4">
            ...
            {posts.map((post) => (
              <article key={post.slug} className="card-hover bg-white rounded-2xl overflow-hidden shadow-sm">
                <div className="h-48 bg-cover bg-center relative" style={{backgroundImage: `url(${post.imagen})`}}>
                  ...
                  <div className="absolute top-4 left-4"><span className={`text-xs font-medium px-3 py-1 rounded-full ${categoriaColor[post.categoria]}`}>{post.categoria}</span></div>
                </div>
                <div className="p-6">
                  <div className="flex items-center gap-3 text-sm text-slate-500 mb-3"><span>{post.fecha}</span><span>-</span><span>{post.minutos} min</span></div>
                  <h2 className="text-lg font-bold mb-2 hover:underline" style={{color: 'var(--color-primary)'}}><Link href={`/blog/${post.slug}`}>{post.titulo}</Link></h2>
          ...
          <div className="mt-16 bg-white rounded-2xl p-8 shadow-sm text-center">
            ...
            <Link href="/itinerarios" className="btn-primary inline-flex items-center gap-2 hover:scale-105 transition-all">Ver itinerarios desde 5.99 EUR</Link>
          </div>
        </div>
      </section>
    </main>
